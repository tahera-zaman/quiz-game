...
<?php
	...
	if($_POST){
		...
		$query = "SELECT * FROM answer_clg_gk_option WHERE question_number = $number AND is_correct = 1";
		...
		if($number == $total_questions){
			header("LOCATION: final_gk_clg.php");
		}else{
			header("LOCATION: start_gk_clg.php?n=". $next);
		}
