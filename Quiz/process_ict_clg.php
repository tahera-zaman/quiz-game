...
<?php
	...
	if($_POST){
		...
		$query = "SELECT * FROM answer_clg_ict_option WHERE question_number = $number AND is_correct = 1";
		...
		if($number == $total_questions){
			header("LOCATION: final_ict_clg.php");
		}else{
			header("LOCATION: start_ict_clg.php?n=". $next);
		}
