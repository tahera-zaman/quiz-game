...
<?php
	...
	if($_POST){
		...
		$query = "SELECT * FROM answer_clg_math_option WHERE question_number = $number AND is_correct = 1";
		...
		if($number == $total_questions){
			header("LOCATION: final_math_clg.php");
		}else{
			header("LOCATION: start_math_clg.php?n=". $next);
		}
