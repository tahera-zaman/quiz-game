...
<?php
	...
	if($_POST){
		...
		$query = "SELECT * FROM answer_scl_ict_option WHERE question_number = $number AND is_correct = 1";
		...
		if($number == $total_questions){
			header("LOCATION: final_ict_scl.php");
		}else{
			header("LOCATION: start_ict_scl.php?n=". $next);
		}
