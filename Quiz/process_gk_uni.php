...
<?php
	...
	if($_POST){
		...
		$query = "SELECT * FROM answer_uni_gk_option WHERE question_number = $number AND is_correct = 1";
		...
		if($number == $total_questions){
			header("LOCATION: final_gk_uni.php");
		}else{
			header("LOCATION: start_gk_uni.php?n=". $next);
		}
