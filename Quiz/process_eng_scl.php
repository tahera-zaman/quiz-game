...
<?php
	...
	if($_POST){
		...
		$query = "SELECT * FROM answer_scl_eng_option WHERE question_number = $number AND is_correct = 1";
		...
		if($number == $total_questions){
			header("LOCATION: final_eng_scl.php");
		}else{
			header("LOCATION: start_eng_scl.php?n=". $next);
		}
